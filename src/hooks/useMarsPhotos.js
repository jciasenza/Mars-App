...
export const useMarsPhotos = ({
  ...
  const url =
    mode === "earth_date"
      ? marsApi.byEarthDate(rover, earthDate, page)
      : marsApi.bySol(rover, sol, page);
  ...
  useEffect(() => {
    ...
    // activamos la inteligencia de recuperación y servimos el fallback de alta fidelidad
      if (error || (data && !data.photos)) {
        console.warn(
          `[useMarsPhotos] NASA API falló o está inactiva. Cargando datos de respaldo históricos de Marte para ${rover}.`
        );
        
        const mockResult = mockMarsData.getPhotos(rover, sol, earthDate, "fhaz", page);
    ...
    }, [data, error, loading, rover, sol, earthDate, page]);
